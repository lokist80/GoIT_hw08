...
users = [
    ...
    {'name': 'Dudley', 'birthday': datetime(year=1985, month=3, day=19)},
...
def get_birthdays_per_week(users):
    ...
    for i in range(len(users_nt)):
        ...
        if start_data <= check_data <= end_data:
            if check_data.weekday() in (5, 6):
                check_data += timedelta(days=(7 - check_data.weekday()))

            weekdays_dict[check_data.strftime('%A')].append(users_nt[i][0])
